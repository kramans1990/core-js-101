...
function getFizzBuzz(num) {
  let result = num;
  if (num % 5 === 0 && num % 3 === 0) { result = 'FizzBuzz'; } else if (num % 3 === 0) { result = 'Fizz'; } else if (num % 5 === 0) { result = 'Buzz'; }
...
function isTriangle(a, b, c) {
  let arr = []; arr.push(a); arr.push(b); arr.push(c);
...
function isInsideCircle(circle, point) {
  const Xr = circle.center.x; const Yr = circle.center.y;
  const { x } = point; const { y } = point;
  if (((Xr - x) ** 2 + (Yr - y) ** 2) < (circle.radius ** 2)) { return true; } return false;
}
...
function findFirstSingleChar(str) {
  const arr = str.split('');
  let result = 'null';
  const d = arr.filter((item, index, array) => array.indexOf(item) === index);
  d.forEach((item) => {
    const entries = arr.filter((p) => p === item);
    if (entries.length === 1) {
      if (result === 'null') {
        result = item;
      }
...
function getDigitalRoot(num) {
  ...
  const sumWithInitial = arr.reduce(
    (previousValue, currentValue) => previousValue + 1 * currentValue,
    initialValue,
  );
  if (sumWithInitial > 9) {
    return getDigitalRoot(sumWithInitial);
